...
export default function TVDetailsClient({ show }: { show: any }) {
  ...
  const trackWatchHistory = async (userId: string) => {
    ...
    await supabase.from("watch_history").upsert(
      {
        user_id: userId, media_id: show.id, media_type: "tv",
        title: show.name, poster_path: show.poster_path,
        vote_average: show.vote_average, watched_at: new Date().toISOString(),
  ...
  const toggleFavorite = async () => {
    ...
    if (isFavorite) {
      await supabase.from("favorites").delete()
        .eq("user_id", user.id).eq("media_id", show.id).eq("media_type", "tv");
      setIsFavorite(false);
    } else {
      const genre_ids: number[] = show.genres?.map((g: any) => g.id) ?? [];
      await supabase.from("favorites").insert({
        ...
        title: show.name, poster_path: show.poster_path,
        vote_average: show.vote_average, genre_ids,
      });
      setIsFavorite(true);
    }
  };
    ...
    <div className="bg-[#0a0a0a] text-white">
      ...
      <div className="px-4 sm:px-6 md:px-10 lg:px-16 -mt-24 sm:-mt-32 md:-mt-40 relative z-10">
        <div className="flex flex-col sm:flex-row gap-6 md:gap-10">
          ...
          >
            <img src={`${posterBaseUrl}${show.poster_path}`} alt={show.name}
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4 text-sm">
              <span className="bg-yellow-500 text-black px-2.5 py-0.5 rounded-md font-bold flex items-center gap-1">
                <FaStar size={11} />{show.vote_average?.toFixed(1)}
              </span>
          ...
          >
            ...
            <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 mb-12">
              ...
              <div className="lg:w-64 xl:w-72 shrink-0 space-y-4">
                <div>
                  <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">First Air Date</p>
                  <p className="text-white text-sm font-medium">{show.first_air_date?.split("-")[0]}</p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Seasons</p>
                  <p className="text-white text-sm font-medium">{show.number_of_seasons}</p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Episodes</p>
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {show.genres?.map((genre: any) => (
                      <span key={genre.id} className="bg-red-600 px-2 py-0.5 rounded-full text-xs text-white">
                        {genre.name}
                      </span>
                    ))}
                  </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <textarea value={review} onChange={(e) => setReview(e.target.value)}
                    placeholder="Write your thoughts about this show..." rows={4}
