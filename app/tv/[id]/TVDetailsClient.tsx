...
export default function TVDetailsClient({ show }: { show: any }) {
  ...
  const trackWatchHistory = async (userId: string) => {
    // Extract genre_ids from show.genres (array of {id, name} objects)
    const genre_ids: number[] = show.genres?.map((g: any) => g.id) ?? [];

    // TV shows don't have a single runtime — use episode_run_time[0] or default 45min
    ...
    await supabase.from("watch_history").upsert(
      {
        ...
        genre_ids,   // e.g. [18, 10765]
        runtime,     // minutes per episode
        progress: 100, // visiting the detail page = fully registered as watched
      },
  ...
  const toggleFavorite = async () => {
    ...
    if (isFavorite) {
      ...
      // Also save genre_ids to favorites so profile can compute favorite genre from both sources
      const genre_ids: number[] = show.genres?.map((g: any) => g.id) ?? [];
